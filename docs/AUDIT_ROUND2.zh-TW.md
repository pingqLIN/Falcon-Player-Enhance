...
| File | 類型 | 具體內容 |
|------|------|---------|
| `extension/assets/icons/icon.svg` | aria-label | `aria-label="Shield Pro logo"` |
| `extension/content/anti-popup.js` | header + log | `// Shield Pro - Anti-Popup` + `[Shield Pro] 相容模式啟用` |
| `extension/content/cosmetic-filter.js` | header + log | `// Shield Pro - Cosmetic Filter` + `[Shield Pro] 已啟動` |
| `extension/content/fake-video-remover.js` | 行內 comment | `// 已被 Shield Pro 偵測為播放器` |
| `extension/sandbox/sandbox.js` | header | `// Shield Pro - Sandbox Page Controller` |
| `extension/content/overlay-remover.js` | 行內 comment | `// Shield Pro 內部元素` |
| `extension/rules/noop.js` | header + **MAIN world log** | `console.log('Shield Pro: Anti-Adblock variables mocked.')` |
| `extension/security/url-checker.js` | header | `// Shield Pro - URL Security Checker` |
| `extension/content/player-overlay-fix.css` | comment | `Shield Pro - Player Overlay Fix CSS` |

**可能風險**

- `noop.js` 的 `console.log('Shield Pro...')` 在**目標網站 MAIN world** 執行，開啟 devtools 即可見，是唯一的「對外公開」品牌殘留
...
| F4 | 🟡 MEDIUM | 9 個檔案 | Shield Pro 品牌殘留（含 MAIN world console.log） |
...
| **#2** | **F4 — noop.js `console.log('Shield Pro...')` 優先清除** | 唯一在目標網站 MAIN world 公開輸出的舊品牌字串，其餘 8 處影響有限 |
