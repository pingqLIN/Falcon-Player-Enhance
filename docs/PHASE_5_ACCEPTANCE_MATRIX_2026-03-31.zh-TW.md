...
| G-01 | Contract | Companion / Standalone / AI-Expanded | getSiteRegistry contract, release gate unification | runtime 取得 `getSiteRegistry` 回傳並核對必要欄位 + `shield-basic-docidle` excludeMatches contract + `filter-rules.json` lovable allow rule contract + `tests/live-browser/run_live_browser_target_contract_regression.py` | 必要欄位皆存在，缺值時 fallback 符合規格；`lovable.dev` 與 `auth.lovable.dev` 不再吃到全站基礎腳本與錯誤 DNR 阻擋；live-browser smoke target manifest 必須維持 repo-relative curated lineage、保留 `requiresManualReview`，且 smoke subset 不可脫離 curated pool |
...
| G-06 | Whitelist Consistency | Companion / Standalone | whitelist-state divergence | `python tests/site-state/run_site_state_consumer_contract_regression.py` + `python tests/site-state/run_site_state_bridge_regression.py --headless` + `python tests/site-state/run_site_state_helper_regression.py --headless` + `python tests/site-state/run_site_state_consistency_regression.py --headless` + `python tests/site-state/run_player_controls_site_state_regression.py --headless` + `python tests/anti-antiblock/run_anti_antiblock_whitelist_regression.py --headless` | MAIN-world bridge、canonical helper、doc-idle media automation consumer 與 anti-antiblock 行為一致；bridge payload 必須輸出 `siteStateHydrated`；helper content script 必須先於 media automation consumers 注入，且 consumer 不得回退到繞過 `interactionSafety` / `mediaAutomationExcludedDomains` 的舊 whitelist-only fallback；non-whitelist / whitelist / strict-mode 與 whitelist restore（strict -> whitelist）切換皆符合預期，且同頁 whitelist -> strict 轉換時 player detector / enhancer / overlay-remover / fake-video-remover 與 player-controls speed UI 必須同步收斂 |
