...
| G-06 | Whitelist Consistency | Companion / Standalone | whitelist-state divergence | `python tests/site-state/run_site_state_bridge_regression.py --headless` + `python tests/site-state/run_site_state_helper_regression.py --headless` + `python tests/site-state/run_site_state_consistency_regression.py --headless` + `python tests/site-state/run_player_controls_site_state_regression.py --headless` + `python tests/anti-antiblock/run_anti_antiblock_whitelist_regression.py --headless` | MAIN-world bridge、canonical helper、doc-idle media automation consumer 與 anti-antiblock 行為一致；bridge payload 必須輸出 `siteStateHydrated`；non-whitelist / whitelist / strict-mode 與 whitelist restore（strict -> whitelist）切換皆符合預期，且同頁 whitelist -> strict 轉換時 player detector / enhancer / overlay-remover / fake-video-remover 與 player-controls speed UI 必須同步收斂 |
